...
def pose_err(est_pose, gt_pose):
    ...
    orient_err = orient_err.squeeze(1).squeeze(1)
    return posit_err, orient_err

def get_labels(input_pose, gt_pose, thr_x, thr_q):
    posit_err, orient_err = pose_err(input_pose, gt_pose)
    label_x = (posit_err <= thr_x).to(dtype=torch.int64)
    orient_err[torch.isnan(orient_err)] = 0
    label_q = (orient_err <= thr_q).to(dtype=torch.int64)
    return label_x, label_q, posit_err, orient_err
...
# Augmentations
transforms = {
    'train_baseline': transforms.Compose([transforms.ToPILImage(),
    ...
    'test_baseline': transforms.Compose([transforms.ToPILImage(),
                                    transforms.Resize(256),
